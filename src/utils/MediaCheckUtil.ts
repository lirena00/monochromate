  ...
  try {
    ...
    if (isDirectMediaUrl(url)) {
      mediaCheckCache.set(url, {
        ...
        timestamp: now,
        mediaCount: 1,
        type: isDirectImageUrl(url) ? "image" : "video",
      });
      return true;
    }
    ...
    if (totalMediaCount <= 3) {
      ...
      const hasMediaInTitle =
        document.title.toLowerCase().includes("image") ||
      ...
      if (images.length > 0) {
        const img = images[0];
        isLargeMedia =
          img && (img.naturalWidth > 400 || img.naturalHeight > 400);
      ...
      if (videos.length > 0) {
        ...
        isLargeMedia =
          isLargeMedia ||
          (video && (video.videoWidth > 400 || video.videoHeight > 400));
